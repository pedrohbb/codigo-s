...
def sort_and_check(cards):
    global numb, numb_select

    #sort
    x = random.choice(numb)
    numb_select.append(x)
    numb.remove(x)
...
def players():
    ...
    while True:
        try:
            a = 1#int(input("Extract a new random card_? 1 ---> Yes / 0 ---> No : "))
            if a in {0,1}:
                if bool(a):
                    card_ = card()
...
def announce(plyrs, winners, endcards):
    ...
    if len(numb_select) <= 5 or len(numb_select) > 70:
...
player = players()
while count <= 100000:
    mutab_cards = deepcopy(player)
    winners, mutab_cards = rounds(mutab_cards)
    announce(player, winners, mutab_cards)
    count += 1

print(min(qtd), max(qtd), sum(qtd)/len(qtd))
